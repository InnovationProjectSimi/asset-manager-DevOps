...
def viewAssets():
    ...
    return render_template("viewAssets.html", assets=assetList, userAssets=userAssets, listSize=len(assetList))
...
def addAsset():
    ...
    if request.method == "POST":
        ...
        existingAsset = Asset.query.filter_by(name=(formData.get("assetName").lower())).first()
        ...
        # Check if the asset already exists
            ...
            userExistingAsset = UserAsset.query.filter_by(assetID=existingAsset.id, userID=current_user.id).first()
            ...
            if userExistingAsset:
                ...
                userExistingAsset.quantity = userExistingAsset.quantity + int(formData.get("quantity"))
                # Commit to the database
                appDB.session.commit()
                current_app.logger.info("Add asset success - asset already owned, updated quantity")
                # Notify the user
                flash("You already own this asset, you quantity of this asset had been added and updated.", category="Information")
            # The asset already exists but is not owned by the user
                ...
                existingAsset.description = "{}. {}".format(existingAsset.description, formData.get("description"))
                dateFormat = datetime.datetime(*[int(v) for v in formData.get("date").replace('T', '-').replace(':', '-').split('-')])
                new_userAsset = UserAsset(userID=current_user.id, assetID=existingAsset.id, dateStored=dateFormat, quantity=formData.get("quantity"))
                # Add and commit to the database
                appDB.session.add(new_userAsset)
                appDB.session.commit()
                current_app.logger.info("Add asset success - asset exists, updated description")
                # Notify the user
                flash("This asset already exists, you description has been added onto the existing asset and added to your user.", category="Information")
        
        # If the entered data passes all checks    
        else:
            # Add the asset to the database
            new_asset = Asset(name=(formData.get("assetName")).lower(), description=formData.get("description"))
            appDB.session.add(new_asset)
            appDB.session.commit()
            # Add the user asset to the database
            dateFormat = datetime.datetime(*[int(v) for v in formData.get("date").replace('T', '-').replace(':', '-').split('-')])
            new_userAsset = UserAsset(userID=current_user.id, assetID= new_asset.id, dateStored=dateFormat, quantity=formData.get("quantity"))
            appDB.session.add(new_userAsset)
            appDB.session.commit()
...
def updateAsset():
    ...
    if request.method == "POST":
        ...
        return redirect(url_for("views.updateSelectedAsset", name=request.form.get("assetList")))
    ...
    return render_template("updateAsset.html", user=current_user, UserAsset=UserAsset, Assets=assets, name="", description="")
...
def updateSelectedAsset(name):  
    ...
    # Check if the request is a post - A submitted form
    if request.method == "POST":
        existingAsset = Asset.query.filter_by(name=(request.form.get("newName")).lower()).first()
        # Validation of the form data
        ...
        # Check the asset is not set to an already existing asset
            ...
            # Notify the user of the problem
            flash("Asset name cannot be renamed to an existing asset", category="Information")
        # If the entered data passes all checks    
        else:
...
def deleteAsset():  
    ...
    if request.method == "POST" and current_user.username == "admin":
        ...
        if "userAssetList" in request.form:
            ...
            flash("UserAsset {} has been removed".format(request.form.get("userAssetList")), category="Success")
        else:
            ...
            deleteUserAssetList = UserAsset.query.filter_by(assetID=deleteAsset.id).all()
