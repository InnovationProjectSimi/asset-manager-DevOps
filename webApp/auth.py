...
def signUp():
    ...
    if request.method == "POST":
        ...
        if user:
            ...
            flash("User already exists with this username, please choose another username", category="Information")
        elif len(username) < 4:
            ...
            current_app.logger.warning("Signup failed - password does not include symbols")
            flash("The password must include at least one of the following symbols: !, @, #, $, £", category="Information")
        elif not any(char.isdigit() for char in password):
            # If the password does not include a digit then notify the user
            current_app.logger.warning("Signup failed - password does not include a digit")
            flash("The password must include at least one digit", category="Information")
        elif not any(char.isupper() for char in password):
            # If the password does not include an uppercase letter then notify the user
            current_app.logger.warning("Signup failed - password does not include an uppercase character")
            flash("The password must include at least one uppercase letter", category="Information")
        else:
